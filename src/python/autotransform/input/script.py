...
+diff --git a/src/python/autotransform/input/script.py b/src/python/autotransform/input/script.py
+--- a/src/python/autotransform/input/script.py
++++ b/src/python/autotransform/input/script.py
+@@ -47,7 +47,11 @@ class ScriptInput(Input):
+                 replaced_cmd,
+                 capture_output=True,
+                 encoding="utf-8",
+-                check=False,
...
++                event_handler.handle(VerboseEvent({"message": f"Error occurred while running the script: {e}"}))
++                raise
++            
+             stdout_message = f"STDOUT:
+{proc.stdout.strip()}" if proc.stdout.strip() != "" and uses_result_file else "No STDOUT"
+             stderr_message = f"STDERR:
+{proc.stderr.strip()}" if proc.stderr.strip() != "" else "No STDERR"
